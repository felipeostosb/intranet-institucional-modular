...
-- 2. CREACIÓN DE ROLES Y USUARIOS (PLANTILLA CON VARIABLES DE ENTORNO / PLACEHOLDERS)
DO $$
BEGIN
    -- Usuario Core Engine
    ...
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'user_equipo02') THEN
        CREATE USER user_equipo02 WITH ENCRYPTED PASSWORD 'CHANGE_ME_PASSWORD_EQUIPO02';
    END IF;

    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'user_equipo03') THEN
        CREATE USER user_equipo03 WITH ENCRYPTED PASSWORD 'CHANGE_ME_PASSWORD_EQUIPO03';
    END IF;

    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'user_equipo04') THEN
        CREATE USER user_equipo04 WITH ENCRYPTED PASSWORD 'CHANGE_ME_PASSWORD_EQUIPO04';
    END IF;

    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'user_equipo05') THEN
        CREATE USER user_equipo05 WITH ENCRYPTED PASSWORD 'CHANGE_ME_PASSWORD_EQUIPO05';
    END IF;

    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'user_equipo06') THEN
        CREATE USER user_equipo06 WITH ENCRYPTED PASSWORD 'CHANGE_ME_PASSWORD_EQUIPO06';
    END IF;

    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'user_equipo07') THEN
        CREATE USER user_equipo07 WITH ENCRYPTED PASSWORD 'CHANGE_ME_PASSWORD_EQUIPO07';
    END IF;

    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'user_equipo08') THEN
        CREATE USER user_equipo08 WITH ENCRYPTED PASSWORD 'CHANGE_ME_PASSWORD_EQUIPO08';
    END IF;

    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'user_equipo09') THEN
        CREATE USER user_equipo09 WITH ENCRYPTED PASSWORD 'CHANGE_ME_PASSWORD_EQUIPO09';
    END IF;
END $$;
